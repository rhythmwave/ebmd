...
class KibADataTable extends DataTable
{
    /**
     ...
     */
    protected string $dataTableVariable = 'dataTableKIBA';

    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        // return (new EloquentDataTable($query))
        //     ->addColumn('action', 'kiba.action')
        //     ->setRowId('id');
    }
     ...
     */
    public function query(KibA $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
                    ->setTableId('kiba-table')
                    ->columns($this->getColumns())                    
                    // ->ajax(route('order.datakiba'))
                    ->dom('tip')
                    ->deferLoading(false)
                    ->orderBy(1)
                    ->selectStyleSingle()
